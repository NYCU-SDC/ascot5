 ...
 * @param p0 particle_simd_fo struct for storing previous state
 * @param h pointer to array containing time steps
 * @param Bdata pointer to magnetic field data
 * @param Edata pointer to electric field data
 * @param sim pointer to simulation data
 * @param cputime wall-clock time elapsed since last cputime_last update
 * @param fuse_endcond if non-zero, call endcond_check_fo_particle inline
 * @param aldforce indicates whether Abraham-Lorentz-Dirac force is enabled
 * @param reverse_time if non-zero, integrate backwards in time
 */
 ...
 *
 * Same as step_fo_vpa but with MHD perturbations included.
 *
 * @param p particle_simd_fo struct that will be updated
 * @param p0 particle_simd_fo struct for storing previous state
 * @param h pointer to array containing time steps
 ...
 * @param cputime wall-clock time elapsed since last cputime_last update
 * @param fuse_endcond if non-zero, call endcond_check_fo_particle inline
 * @param aldforce reserved, not implemented for MHD path
 * @param reverse_time if non-zero, integrate backwards in time
 */
void step_fo_vpa_mhd(
    ...
    #pragma acc parallel loop present(p[0:1], p0[0:1], h[0:NSIMD], \
                                      Bdata[0:1], Edata[0:1], boozer[0:1], \
                                      mhd[0:1], sim[0:1])
#endif
